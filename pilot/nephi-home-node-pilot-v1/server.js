...
function createRequestHandler(service, options = {}) {
  ...
  const roomGalleryRoutes = require("./lib/room-gallery-routes").createRoomGalleryRoutes({
    getStore: options.getRoomGalleryStore || (async () => { throw new AppError(503, "ROOM_GALLERY_UNAVAILABLE", "房間照片尚未啟用"); }),
    persistence, customerSettings, publicBaseUrl: publicBrand.publicBaseUrl, sendData
  });
  ...
  return async function handleRequest(request, response) {
    ...
    try {
      if (await roomGalleryRoutes(request, response, url)) return;
      ...
      if (request.method === "GET" && pathname === "/admin/platform") {const token=cookieValue(request,"nephi_admin_session"),session=token&&adminAuthRequired?await persistence.getAdminSession(sessionTokenHash(token)):null;if(!session||!onboarding||!onboarding.isPlatformAdmin(session))throw new AppError(401,"PLATFORM_ADMIN_REQUIRED","需要平台管理者權限");return sendStatic(response,"admin-platform.html",publicBrand);}
...
function createApp(options = {}) {
  ...
  const getRoomGalleryStore = options.getRoomGalleryStore || roomGalleryRuntime.getStore;
