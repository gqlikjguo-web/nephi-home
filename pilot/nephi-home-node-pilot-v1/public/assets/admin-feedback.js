...
(()=>{
 ...
 function build(){
  host.replaceChildren();const header=el('div','feedback-heading');header.append(el('p','eyebrow','傾聽每一次入住'),el('h2','','住客回饋'),el('p','hint','看見值得保持的好，也把需要改善的事一步步做好。'));host.append(header);
  ...
  const share=el('section','feedback-card feedback-share'),shareText=el('div');shareText.append(el('h3','','我的住客回饋表'),el('p','hint','這個連結可長期使用，適合分享給客人或印在房間與退房說明。'));
  const link=el('a','feedback-url');link.dataset.feedback='url';link.target='_blank';link.rel='noopener noreferrer';
  const copy=el('button','','複製連結');copy.type='button';copy.onclick=async()=>{try{await navigator.clipboard.writeText(link.href);$('message').textContent='已複製回饋表連結';}catch{$('message').textContent='請長按上方連結複製';}};
  shareText.append(link,copy);const qr=el('img');qr.dataset.feedback='qr';qr.alt='掃描填寫住宿回饋';qr.width=180;qr.height=180;share.append(shareText,qr);host.append(share);
  ...
  const experiences=el('section','feedback-card');experiences.append(el('h3','','各項體驗'),el('p','hint','依全部回饋計算；未填的項目不列入平均。'));const bars=el('div','feedback-experiences');bars.dataset.feedback='categories';experiences.append(bars);host.append(experiences);
  const ranks=el('div','feedback-ranks');for(const [key,title] of [['positives','客人最滿意'],['improvements','最常希望改善']]){const section=el('section','feedback-card');section.append(el('h3','',title));const list=el('ol','feedback-ranking');list.dataset.feedback=key;section.append(list);ranks.append(section);}host.append(ranks);
 ...
 function summary(data){const metrics=$('metrics');metrics.replaceChildren();for(const [title,value,hint] of [['整體平均評分',number(data.average),'全部回饋 · 滿分 5 分'],['本月收到',data.monthCount,'份回饋'],['尚未查看',data.unviewed,'份待查看'],['待改善',data.needsImprovement,'份待處理']]){const card=el('article','feedback-metric');card.append(el('span','',title),el('strong','',String(value)),el('small','hint',hint));metrics.append(card);}
  ...
  for(const [key,title] of Object.entries(labels)){const value=data.categories[key],row=el('div','feedback-experience'),top=el('div');top.append(el('span','',title),el('strong','',number(value)+(value!==null&&high!==low?(value===high?' · 最高':value===low?' · 最低':''):'')));const meter=el('meter');meter.min=0;meter.max=5;meter.value=value||0;meter.setAttribute('aria-label',title+'平均分');row.append(top,meter);bars.append(row);}
  for(const [key,names] of [['positives',positive],['improvements',improvement]]){const list=$(key);list.replaceChildren();if(!data[key].length){list.append(el('p','hint','還沒有相關回饋'));continue;}for(const item of data[key]){const li=el('li');li.append(el('span','',names[item.key]),el('strong','',item.count+' 次'));list.append(li);}}
 }
 function card(item){const a=el('article','feedback-card feedback-entry'),top=el('div','feedback-entry-top');top.append(el('strong','feedback-stars','★'.repeat(item.overall)+'☆'.repeat(5-item.overall)+' '+number(item.overall)),el('span','feedback-badge',statuses[item.status]));a.append(top,el('p','hint',date(item.created_at)));
  if(!item.stay_date&&!item.room_name)a.append(el('p','feedback-anonymous','匿名回饋'));else{if(item.stay_date)a.append(el('p','','入住：'+item.stay_date.replaceAll('-','/')));if(item.room_name)a.append(el('p','','房型：'+item.room_name));}
  const ratings=el('div','feedback-rating-chips');for(const [key,title] of Object.entries(labels))if(item.ratings[key])ratings.append(el('span','',title+' '+item.ratings[key]));a.append(ratings);
  for(const [title,value] of [['最滿意',item.positives.map(k=>positive[k]).join('、')],['希望改善',item.improvements.map(k=>improvement[k]).join('、')],['客人留言',item.comment],['再次入住',revisits[item.revisit]]])if(value){const p=el('div','feedback-answer');p.append(el('strong','',title),el('p','',value));a.append(p);}
  const edit=el('form','feedback-edit'),state=select('entryStatus',Object.entries(statuses));state.value=item.status;const note=el('textarea');note.rows=2;note.maxLength=1000;note.value=item.internal_note;note.placeholder='例如：9/25 已增加浴室置物架';const save=el('button','','儲存處理狀態'),message=el('p','message');message.setAttribute('role','status');edit.append(field('處理狀態',state),field('內部改善備註（只有業者可見）',note),save,message);
  edit.onsubmit=async e=>{e.preventDefault();const current=generation;save.disabled=true;try{await request('/'+item.id,{method:'PATCH',body:JSON.stringify({status:state.value,internalNote:note.value})});if(current!==generation)return;top.querySelector('.feedback-badge').textContent=statuses[state.value];message.textContent='已儲存';const data=await request('/summary');if(current===generation)summary(data);}catch(e){message.textContent=e.message;}finally{save.disabled=false;}};a.append(edit);return a;
 }
