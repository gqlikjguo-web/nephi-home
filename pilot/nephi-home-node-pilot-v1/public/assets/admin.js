...
let propertyFacts = [];
let customReplies = [];
let selfCheckInOutSetting = { applicableMonth: null, validUntil: null, content: "", enabled: false };
...
function populateSelfCheckInOutCard(){if(!$('selfCheckInOutForm'))return;$("selfCheckInOutApplicableMonth").value=selfCheckInOutSetting.applicableMonth||"";$("selfCheckInOutValidUntil").value=selfCheckInOutSetting.validUntil||"";$("selfCheckInOutContent").value=selfCheckInOutSetting.content||"";$("selfCheckInOutEnabled").checked=selfCheckInOutSetting.enabled===true;}
function selfCheckInOutCard(){const row=element("article","equipment-fact-row"),title=element("h4","","自助入住／退房說明"),fields=element("div","property-fact-grid"),month=document.createElement("select"),validUntil=document.createElement("input"),content=document.createElement("textarea"),enabled=document.createElement("input"),enabledLabel=element("label","inline","啟用");fields.id="selfCheckInOutForm";month.id="selfCheckInOutApplicableMonth";month.append(new Option("不限月份",""),...Array.from({length:12},(_,index)=>new Option(`${index+1} 月`,String(index+1))));validUntil.id="selfCheckInOutValidUntil";validUntil.type="date";content.id="selfCheckInOutContent";content.rows=3;content.maxLength=2000;content.placeholder="可直接回覆旅客的正式說明";enabled.id="selfCheckInOutEnabled";enabled.type="checkbox";enabledLabel.prepend(enabled);fields.append(propertyFactField("適用入住月份",month),propertyFactField("說明使用至",validUntil),propertyFactField("正式對客說明",content),enabledLabel);row.append(title,fields);return row;}
...
async function loadProfile() {
  ...
  $("profileName").value = profile.propertyName || ""; $("profileAiName").value = profile.aiName || ""; $("profileAddress").value = profile.address || ""; $("profileGoogleMapsUrl").value = profile.googleMapsUrl || ""; $("profileLineUrl").value = profile.lineUrl || ""; $("profileContactInfo").value = profile.contactInfo || ""; $("profileCheckInTime").value = profile.checkInTime || ""; $("profileEarlyCheckInPolicy").value = profile.earlyCheckInPolicy || ""; $("profileLatestArrivalTime").value = profile.latestArrivalTime || ""; $("profileCheckOutTime").value = profile.checkOutTime || ""; renderAvailabilityAutoReply(profile.availabilityAutoReplyEnabled !== false);
  selfCheckInOutSetting=profile.selfCheckInOutInstructions||{applicableMonth:null,validUntil:null,content:"",enabled:false};populateSelfCheckInOutCard();
}

function propertyProfilePayload() { return { propertyId: session.propertyId, propertyName: $("profileName").value, aiName: $("profileAiName").value, address: $("profileAddress").value, googleMapsUrl: $("profileGoogleMapsUrl").value, lineUrl: $("profileLineUrl").value, contactInfo: $("profileContactInfo").value, checkInTime: $("profileCheckInTime").value, earlyCheckInPolicy: $("profileEarlyCheckInPolicy").value, latestArrivalTime: $("profileLatestArrivalTime").value, checkOutTime: $("profileCheckOutTime").value, availabilityAutoReplyEnabled: $("availabilityAutoReplyOn").dataset.selected === "true" }; }
function selfCheckInOutPayload(){return{applicableMonth:$("selfCheckInOutApplicableMonth").value||null,validUntil:$("selfCheckInOutValidUntil").value||null,content:$("selfCheckInOutContent").value,enabled:$("selfCheckInOutEnabled").checked};}
