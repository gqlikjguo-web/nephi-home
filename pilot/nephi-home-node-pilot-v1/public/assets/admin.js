...
function installSelfCheckInOutForm(){const profile=document.querySelector(".other-settings .profile-card"),section=document.createElement("section");section.className="profile-card";section.innerHTML='<h2>自助入住／退房說明</h2><form id="selfCheckInOutForm"><label>適用入住月份<select id="selfCheckInOutApplicableMonth"><option value="">不限月份</option>'+Array.from({length:12},(_,index)=>`<option value="${index+1}">${index+1} 月</option>`).join("")+'</select></label><p class="hint">不選月份代表不限入住月份。</p><label>說明使用至<input id="selfCheckInOutValidUntil" type="date"></label><p class="hint">不填代表長期有效。</p><label>回覆內容<textarea id="selfCheckInOutContent" maxlength="2000" rows="5"></textarea></label><label class="inline"><input id="selfCheckInOutEnabled" type="checkbox">啟用</label><div class="actions"><button type="submit">儲存</button></div><p id="selfCheckInOutStatus" class="message" role="status" aria-live="polite"></p></form>';profile.after(section);}
installSelfCheckInOutForm();
...
async function loadProfile() {
  ...
  const instructions=profile.selfCheckInOutInstructions||{};$("selfCheckInOutApplicableMonth").value=instructions.applicableMonth||"";$("selfCheckInOutValidUntil").value=instructions.validUntil||"";$("selfCheckInOutContent").value=instructions.content||"";$("selfCheckInOutEnabled").checked=instructions.enabled===true;
}
...
$("selfCheckInOutForm").onsubmit=async event=>{event.preventDefault();const form=event.currentTarget;if(!form.reportValidity())return;const status=$("selfCheckInOutStatus");status.textContent="儲存中…";try{const profile=await api("/api/property-profile",{method:"PUT",body:JSON.stringify({...propertyProfilePayload(),selfCheckInOutInstructions:selfCheckInOutPayload()})});const instructions=profile.selfCheckInOutInstructions||{};$("selfCheckInOutApplicableMonth").value=instructions.applicableMonth||"";$("selfCheckInOutValidUntil").value=instructions.validUntil||"";$("selfCheckInOutContent").value=instructions.content||"";$("selfCheckInOutEnabled").checked=instructions.enabled===true;status.textContent="已儲存";}catch(error){status.textContent=`儲存失敗：${error.message}。輸入內容仍保留，請重試。`;}};
