...
function renderBundleAmenities(items=[]){const byKey=new Map(items.map(item=>[item.key,item]));const nodes=bundleAmenityPresets.map(([key,name])=>{const item=byKey.get(key)||{},status=item.provided===true?"yes":item.provided===false&&item.statusSource==="operator"?"no":"unknown",row=element("div","amenity-row"),select=document.createElement("select"),label=document.createElement("label"),noteLabel=document.createElement("label"),note=document.createElement("input");row.dataset.amenityKey=key;for(const [value,text] of [["unknown","未知"],["yes","有"],["no","沒有"]]){const option=document.createElement("option");option.value=value;option.textContent=text;select.append(option)}select.value=status;select.dataset.amenityStatus="";label.append(document.createTextNode(name),select);note.maxLength=100;note.value=status==="yes"?item.note||"":"";note.dataset.amenityNote="";noteLabel.append(document.createTextNode("備註（選填）"),note);noteLabel.hidden=status!=="yes";select.onchange=()=>{noteLabel.hidden=select.value!=="yes";if(select.value!=="yes")note.value=""};row.append(label,noteLabel);return row});for(const item of items.filter(item=>item.source==="custom"&&item.provided)){const row=element("div","custom-amenity-row"),name=document.createElement("input"),note=document.createElement("input"),remove=cell("button","刪除");name.value=item.displayName||"";name.maxLength=20;name.dataset.customAmenityName="";note.value=item.note||"";note.maxLength=100;note.dataset.customAmenityNote="";remove.type="button";remove.className="secondary";remove.onclick=()=>row.remove();row.append(name,note,remove);nodes.push(row)}$("bundleAmenities").replaceChildren(...nodes)}
function collectBundleAmenities(){const presets=[...$("bundleAmenities").querySelectorAll("[data-amenity-key]")].map((row,position)=>{const input=row.querySelector("[data-amenity-status]"),preset=bundleAmenityPresets[position],provided=input.value==="yes"?true:input.value==="no"?false:null;return{key:preset[0],displayName:preset[1],provided,statusSource:provided===null?null:"operator",note:provided===true?row.querySelector("[data-amenity-note]").value:"",source:"preset",position}});const custom=[...$("bundleAmenities").querySelectorAll(".custom-amenity-row")].map((row,index)=>({key:"",displayName:row.querySelector("[data-custom-amenity-name]").value,provided:true,note:row.querySelector("[data-custom-amenity-note]").value,source:"custom",position:bundleAmenityPresets.length+index}));return presets.concat(custom)}
