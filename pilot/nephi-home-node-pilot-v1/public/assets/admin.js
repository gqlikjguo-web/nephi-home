...
$("firstSetupForm").onsubmit = async event => { event.preventDefault(); const form = event.currentTarget, message = $("firstSetupMessage"), button = form.querySelector("button"); button.disabled = true; message.textContent = "寄送中…"; try { await api("/api/admin/setup-link", { method: "POST", body: JSON.stringify(Object.fromEntries(new FormData(form))) }); message.textContent = "若此 Email 有有效的首次設定資格，設定連結將寄到您的信箱。"; } catch { message.textContent = "目前無法寄送，請稍後再試。"; } finally { button.disabled = false; } };
