...
function collectBundleAmenities(){const rows=[...$("bundleAmenities").querySelectorAll("[data-amenity-key]")],byKey=new Map(rows.filter(row=>row.dataset.customAmenity!=="true").map(row=>[row.dataset.amenityKey,row]));const presets=bundleAmenityPresets.map(([key,displayName],position)=>{const row=byKey.get(key),status=row?.dataset.amenityStatus||"unknown",provided=status==="yes"?true:status==="no"?false:null;return{key,displayName,provided,statusSource:provided===null?null:"operator",note:provided===true?row.dataset.amenityNote||"":"",source:"preset",position}});const custom=rows.filter(row=>row.dataset.customAmenity==="true"&&row.dataset.amenityStatus==="yes").map((row,index)=>({key:"",displayName:row.dataset.amenityName,provided:true,note:row.dataset.amenityNote||"",source:"custom",position:bundleAmenityPresets.length+index}));return presets.concat(custom)}
...
function renderMembers() {
  $("memberRooms").replaceChildren(...rooms.filter(room => room.inventoryType !== "bundle").map(room => {
    const label = document.createElement("label"), input = document.createElement("input"); input.type = "checkbox"; input.name = "memberRoom"; input.value = room.id; input.disabled = true; label.append(input, document.createTextNode(room.name)); return label;
  }));
}
async function loadBundles() {
  ...
  $("bundleList").replaceChildren(...bundles.map(bundle => {
    ...
    const edit = cell("button", "修改價格");
    edit.className = "secondary"; edit.onclick = () => editBundle(bundle); row.append(details, edit); return row;
  }));
}
...
$("bundleAddCustomAmenity").onclick=()=>{};$("bundleAmenityEditorCancel").onclick=()=>$("bundleAmenityEditor").close();$("bundleAmenityStatus").onchange=syncBundleAmenityEditor;$("bundleAmenityDelete").onclick=()=>{};$("bundleAmenityEditorForm").onsubmit=event=>event.preventDefault();
renderBundleAmenities();
$("bundleForm").onsubmit = async event => { event.preventDefault(); const id = $("bundleId").value, status = $("bundleStatus"); if (!id) { status.textContent = "方案識別不存在，無法儲存"; return; } const payload = { customerId: session.propertyId, mondayThursdayPrice: Number($("bundleMondayThursdayPrice").value), fridayPrice: Number($("bundleFridayPrice").value), saturdayHolidayPrice: Number($("bundleSaturdayHolidayPrice").value), sundayPrice: Number($("bundleSundayPrice").value) }; status.textContent = "儲存中…"; try { await api(`/api/bundles/${encodeURIComponent(id)}`, { method: "PUT", body: JSON.stringify(payload) }); await Promise.all([loadBundles(), loadMonth()]); status.textContent = "已儲存"; clearBundle(); } catch (error) { status.textContent = `儲存失敗：${error.message}。輸入內容仍保留，請重試。`; } };
