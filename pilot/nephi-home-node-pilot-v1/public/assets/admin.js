...
function showLogin(message = "") { session = null; noteEditorState = null; $("noteEditor").hidden = true; $("login").hidden = false; $("propertyChooser").hidden = true; $("workspace").hidden = true; $("logout").hidden = true; configureGuestFrontendLinks(""); $("loginMessage").textContent = message; }
function showPropertyChooser(value) {
  session = value; $("login").hidden = true; $("workspace").hidden = true; $("logout").hidden = false; configureGuestFrontendLinks(""); $("propertyChooser").hidden = false; $("propertyChoiceMessage").textContent = "";
...
async function loadProfile() {
  ...
  $("profileName").value = profile.propertyName || ""; $("profileAiName").value = profile.aiName || ""; $("profileAddress").value = profile.address || ""; $("profileGoogleMapsUrl").value = profile.googleMapsUrl || ""; $("profileLineUrl").value = profile.lineUrl || ""; $("profileContactInfo").value = profile.contactInfo || ""; $("profileCheckInTime").value = profile.checkInTime || ""; $("profileLatestArrivalTime").value = profile.latestArrivalTime || ""; $("profileCheckOutTime").value = profile.checkOutTime || "";
}
...
async function enter(value) { if (value.requiresPropertySelection || !value.propertyId) return showPropertyChooser(value); if (expectedSlug) value = await api(`/api/admin/session?slug=${encodeURIComponent(expectedSlug)}`); session = value; $("login").hidden = true; $("propertyChooser").hidden = true; $("workspace").hidden = false; $("logout").hidden = false; configureGuestFrontendLinks(value.propertyId); $("propertyLabel").textContent = `業者：${value.propertyId}`; $("month").value = $("month").value || currentMonth(); let savedView = "recent"; try { savedView = localStorage.getItem("junzanAvailabilityView") || "recent"; } catch {} availabilityState.view = matchMedia("(max-width: 640px)").matches ? "recent" : ["recent", "calendar"].includes(savedView) ? savedView : "recent"; await loadMonth(); await Promise.all([loadBundles(), loadPricing(), loadProfile(), loadPropertyFacts()]); fillCustomReplyOptions();clearCustomReplyForm();await loadCustomReplies(); }
