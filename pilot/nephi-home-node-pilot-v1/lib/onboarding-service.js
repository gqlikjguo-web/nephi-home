...
function cleanOnboardingInput(input={},draft=false){
 ...
 const emailValue=text(input.email,160),checkInValue=text(input.checkInTime,5),latestArrivalValue=text(input.latestArrivalTime,500),checkOutValue=text(input.checkOutTime,5);
 return{propertyName:text(input.propertyName,100),aiName:text(input.aiName,40),contactName:text(input.contactName,80),phone:text(input.phone,40),email:emailValue?email(emailValue):(draft?"":email(emailValue)),address:text(input.address,300),googleMapsUrl:normalizeGoogleMapsUrl(input.googleMapsUrl),checkInTime:checkInValue?validTime(checkInValue):(draft?"":validTime(checkInValue)),latestArrivalTime:latestArrivalValue,checkOutTime:checkOutValue?validTime(checkOutValue):(draft?"":validTime(checkOutValue)),line:{hasOfficialAccount,contactLink:lineContactLink(input.line&&input.line.contactLink,hasOfficialAccount)},propertyIdSuggestion:text(input.propertyIdSuggestion,48),rooms,bundles,propertyFacts,knowledge};
}
...
function createOnboardingService(provider,{emailNotifier}={}){
 ...
 return{
 async requestAdminSetupLink(value){const normalizedEmail=String(value||"").trim().toLowerCase();if(!emailNotifier||!emailNotifier.configured||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail))return{accepted:true};try{const expiresAt=new Date(Date.now()+7*86400000).toISOString(),invitations=provider.issueAdminSetupInvitationsByEmail(normalizedEmail,expiresAt);if(invitations.length)await emailNotifier.sendAdminSetupLinks({recipient:invitations[0].email,invitations});}catch{}return{accepted:true};},
 ...
 async redeemInvitation(token,password,confirmPassword){const tokenHash=sessionTokenHash(text(token,500)),invitation=provider.getAdminInvitation(tokenHash);if(!invitation)throw new AppError(400,"INVALID_ADMIN_INVITATION","邀請碼無效或已使用");let passwordHash="";if(!invitation.existingIdentity){const value=String(password||"");if(value!==String(confirmPassword||""))throw new AppError(400,"PASSWORD_CONFIRMATION_MISMATCH","兩次輸入的新密碼不一致");if(value.length<8||value.length>12)throw new AppError(400,"INVALID_ADMIN_PASSWORD","新密碼必須為 8–12 字元");passwordHash=await hashPassword(value);}try{return provider.redeemAdminInvitation(tokenHash,passwordHash);}catch{throw new AppError(400,"INVALID_ADMIN_INVITATION","邀請碼無效或已使用");}}
};}
