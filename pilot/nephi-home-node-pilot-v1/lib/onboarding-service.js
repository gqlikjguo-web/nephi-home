...
function cleanInput(input={}){
 const rooms=(input.rooms||[]).map((x,i)=>{const displayName=text(x.displayName||x.name,80);if(!displayName)throw new AppError(400,"MISSING_ROOM_DISPLAY_NAME","房型顯示名稱不得空白");const highlights=normalizeRoomHighlights(x.highlights);if(highlights.length>3||highlights.some(item=>characterCount(item)>15))throw new AppError(400,"INVALID_ROOM_HIGHLIGHTS","房型亮點最多 3 項，每項最多 15 字");return{key:text(x.key||`room_${i+1}`,40),roomCode:text(x.roomCode,40),displayName,name:displayName,type:text(x.type,40),capacity:integer(x.capacity,1),highlights,mondayThursdayPrice:integer(x.mondayThursdayPrice),fridayPrice:integer(x.fridayPrice),saturdayHolidayPrice:integer(x.saturdayHolidayPrice),sundayPrice:integer(x.sundayPrice),enabled:x.enabled!==false};});
 const keys=new Set(rooms.map(x=>x.key));if(keys.size!==rooms.length)throw new AppError(400,"DUPLICATE_ROOM_KEY","房型識別重複");
 const bundles=(input.bundles||[]).map((x,i)=>{const members=[...new Set((x.memberRoomKeys||[]).map(v=>text(v,40)))];if(!members.length||members.some(k=>!keys.has(k)))throw new AppError(400,"INVALID_BUNDLE_MEMBER","包棟成員房型錯誤");const legacy=Object.hasOwn(x,"basePrice")?integer(x.basePrice):0,readPrice=key=>Object.hasOwn(x,key)?integer(x[key]):legacy;return{key:text(x.key||`bundle_${i+1}`,40),name:text(x.name,80),memberRoomKeys:members,capacity:integer(x.capacity,1),mondayThursdayPrice:readPrice("mondayThursdayPrice"),fridayPrice:readPrice("fridayPrice"),saturdayHolidayPrice:readPrice("saturdayHolidayPrice"),sundayPrice:readPrice("sundayPrice"),enabled:x.enabled!==false,entertainmentAmenities:normalizeEntertainmentAmenities(x.entertainmentAmenities)};});
 ...
 return{propertyName:text(input.propertyName,100),contactName:text(input.contactName,80),phone:text(input.phone,40),email:email(input.email),address:text(input.address,300),googleMapsUrl:normalizeGoogleMapsUrl(input.googleMapsUrl),checkInTime:validTime(input.checkInTime),checkOutTime:validTime(input.checkOutTime),line:{hasOfficialAccount,contactLink:lineContactLink(input.line&&input.line.contactLink,hasOfficialAccount)},propertyIdSuggestion:text(input.propertyIdSuggestion,48),rooms,bundles,knowledge};
}
