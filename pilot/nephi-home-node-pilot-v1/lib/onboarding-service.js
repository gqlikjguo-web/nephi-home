...
function cleanInput(input={}){
 const rooms=(input.rooms||[]).map((x,i)=>({key:text(x.key||`room_${i+1}`,40),name:text(x.name,80),type:text(x.type,40),capacity:integer(x.capacity,1),mondayThursdayPrice:integer(x.mondayThursdayPrice),fridayPrice:integer(x.fridayPrice),saturdayHolidayPrice:integer(x.saturdayHolidayPrice),sundayPrice:integer(x.sundayPrice),enabled:x.enabled!==false}));
 ...
 return{propertyName:text(input.propertyName,100),contactName:text(input.contactName,80),phone:text(input.phone,40),email:email(input.email),address:text(input.address,300),googleMapsUrl:normalizeGoogleMapsUrl(input.googleMapsUrl),checkInTime:validTime(input.checkInTime),checkOutTime:validTime(input.checkOutTime),line:{hasOfficialAccount:Boolean(input.line&&input.line.hasOfficialAccount),channelId:text(input.line&&input.line.channelId,80),contactLink:text(input.line&&input.line.contactLink,500)},propertyIdSuggestion:text(input.propertyIdSuggestion,48),rooms,bundles,knowledge};
}
