...
function cleanInput(input={}){
 ...
 const bundles=(input.bundles||[]).map((x,i)=>{const members=[...new Set((x.memberRoomKeys||[]).map(v=>text(v,40)))];if(!members.length||members.some(k=>!keys.has(k)))throw new AppError(400,"INVALID_BUNDLE_MEMBER","包棟成員房型錯誤");const legacy=Object.hasOwn(x,"basePrice")?integer(x.basePrice):0,readPrice=key=>Object.hasOwn(x,key)?integer(x[key]):legacy;return{key:text(x.key||`bundle_${i+1}`,40),name:text(x.name,80),memberRoomKeys:members,capacity:integer(x.capacity,1),mondayThursdayPrice:readPrice("mondayThursdayPrice"),fridayPrice:readPrice("fridayPrice"),saturdayHolidayPrice:readPrice("saturdayHolidayPrice"),sundayPrice:readPrice("sundayPrice"),enabled:x.enabled!==false};});
 ...
 return{propertyName:text(input.propertyName,100),contactName:text(input.contactName,80),phone:text(input.phone,40),email:email(input.email),address:text(input.address,300),googleMapsUrl:normalizeGoogleMapsUrl(input.googleMapsUrl),checkInTime:validTime(input.checkInTime),checkOutTime:validTime(input.checkOutTime),line:{hasOfficialAccount,channelId:text(input.line&&input.line.channelId,80),contactLink:lineContactLink(input.line&&input.line.contactLink,hasOfficialAccount)},propertyIdSuggestion:text(input.propertyIdSuggestion,48),rooms,bundles,knowledge};
}
