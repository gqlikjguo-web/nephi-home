...
function createNewCoreManualTestService({ persistence, providers, service, apiKey, now = () => new Date(), executeTurn = executeNewCoreManualTurn } = {}) {
  const repository = new NewCoreManualTestRepository({ persistence, now });
  const resolver = { availability: (query) => service.searchAvailability(query), availableDates: (query) => service.searchAvailableDates(query), priceOverrides: () => providers.customerSettings.listInventoryPriceOverrides(PROPERTY_ID), dateClassifications: () => providers.customerSettings.listDatePriceClassifications(PROPERTY_ID), customReplies: () => providers.customReplies ? providers.customReplies.list(PROPERTY_ID) : [] };
  ...
  async function runTurn(id, session, body = {}) {
    ...
    const property = providers.customerSettings.getProperty(PROPERTY_ID); if (!property) { const error = new Error("property_not_found"); error.code = "PROPERTY_NOT_FOUND"; throw error; }
