...
function plannerJsonSchema() {
  ...
  const evidenceRefSchema = { type: "object", additionalProperties: false, description: "A source-bound evidence coordinate. Identify exactly one supplied current sourceEvents item or, only for an allowed historical semantic candidate, one conversationLineage.userSources item by eventId or messageRef. quote must be the exact JavaScript string slice from that source message.", required: ["eventId", "messageRef", "startOffset", "endOffset", "quote"], properties: { eventId: { type: "string", maxLength: 120, description: "Copy the supplied evidence source eventId verbatim, or use an empty string only when messageRef identifies that source." }, messageRef: { type: "string", maxLength: 120, description: "Copy the supplied evidence source messageRef verbatim, or use an empty string only when eventId identifies that source." }, startOffset: { type: "integer", minimum: 0, description: "0-based UTF-16 JavaScript string offset, inclusive, in the identified source messageText." }, endOffset: { type: "integer", minimum: 0, description: "0-based UTF-16 JavaScript string offset, exclusive, greater than startOffset and no greater than the identified source messageText length." }, quote: { type: "string", minLength: 1, maxLength: 500, description: "Exactly the identified source messageText.slice(startOffset, endOffset), copied without paraphrasing, normalization, or translation." } } };
...
function plannerProviderJsonSchema() {
  ...
  schema.properties.semanticCandidates.description = "A lifecycle ledger. Bound capability candidates cite verified current context-relation indexes with no raw evidence. Bound temporal_pattern and lodging_scope candidates may additionally cite exact bounded historical USER evidence. pending_task coverage candidates retain strict current raw source evidence until repair creates the task and relation.";
  ...
  candidateSchema.properties.evidenceRefs = { ...candidateSchema.properties.evidenceRefs, minItems: 0, description: "Required lifecycle field. Use [] for bound capability candidates. Bound temporal_pattern and lodging_scope candidates may cite exact conversationLineage.userSources evidence. pending_task candidates provide strict current-turn raw source evidence." };
