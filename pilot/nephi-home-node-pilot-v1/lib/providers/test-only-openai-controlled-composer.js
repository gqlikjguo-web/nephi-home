...
class TestOnlyOpenAiControlledComposer {
  ...
  async compose(responsePlan, { lineUserId } = {}) {
    ...
    try {
      ...
      const response = await this.fetchImpl(RESPONSES_URL, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${this.apiKey}` }, signal: controller.signal, body: JSON.stringify({ model: this.model, ...(String(lineUserId || "").trim() ? { safety_identifier: sha256(lineUserId) } : {}), input: [{ role: "system", content: [{ type: "input_text", text: "Return every supplied section exactly once, in the supplied order. Copy taskId, responseMode, and exactText without changing, adding, removing, paraphrasing, or translating any content. Put exactText in the output text field." }] }, { role: "user", content: [{ type: "input_text", text: JSON.stringify(composerPlan) }] }], text: { format: { type: "json_schema", name: "junzan_controlled_reply_v2", strict: true, schema: outputSchema(composerPlan) } } }) });
