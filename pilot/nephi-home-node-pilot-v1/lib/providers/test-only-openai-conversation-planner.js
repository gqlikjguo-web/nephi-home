...
function instructions() {
  return [
    ...
    "Independently of relation classification, express the current guest request as one complete standalone task. When its date or lodging product is stated only in bounded prior USER turns, repeat that semantic candidate in the task and cite the exact original USER span in the matching temporal_pattern or lodging_scope candidate. Never cite or derive a value from an assistant turn. A current explicit date or product always wins, and a current invalid or unresolved date must remain unresolved rather than using history.",
    ...
    "Before tasks, emit a bounded semanticCandidates ledger. Map semantically equivalent wording to the same closed capability or propertyCatalog identity; do not use keyword matching. Always emit both lifecycle arrays. Set coverageStatus bound when the candidate already has a task and context relation: provide provenanceRelationCandidateIndexes for those relations. Capability candidates set evidenceRefs to []. Only temporal_pattern and lodging_scope candidates may cite exact bounded historical USER evidenceRefs from conversationLineage.userSources; never cite assistant text. Set coverageStatus pending_task only for a coverage candidate that has no task yet: set provenanceRelationCandidateIndexes to [] and provide exact current-turn raw evidenceRefs solely to preserve its source provenance until a repair creates its task and relation. The adapter verifies historical evidence against the bounded USER source and active canonical cycle; it never trusts a copied date or product. Do not generate opaque IDs or task-to-ledger ownership; the adapter assigns those only after validating the semantic output. Preserve bundle, room restrictions, guest count, and temporal candidates without deciding facts.",
...
function providerConversationContext(input) {
  ...
  const metadata = {
    ...
    conversationLineage: {
      turns: recentConversation.map(({ historyTurn, eventId, messageRef, createdAt, requestCycleRefs }) => ({ historyTurn, eventId, messageRef, createdAt, cycleCount: requestCycleRefs.length })),
      userSources: recentConversation.map(({ historyTurn, eventId, messageRef, guestMessage, createdAt }) => ({ historyTurn, eventId, messageRef, messageText: guestMessage, createdAt })),
