...
function instructions() {
  return [
    ...
    "A lodging reservation arrangement or booking process request that asks how to arrange, proceed with, or make the reservation must use type booking_request, detailIntent general, requestedOutputs answer, dependsOnStayContext false, and stayCandidate null. Preserve every stated date, nights, guests, room, or bundle in sourceText so deterministic canonical validation retains the evidence; booking_request remains a human handoff and must never query availability. A duration alone, without semantic lodging-reservation intent, must not become booking_request.",
