...
function instructions() {
  return [
    ...
    "A fee request for an identified current-property formal catalog subject must use the subject-compatible type amenity, policy, or property_fact with detailIntent fee, requestedOutputs [fee], dependsOnStayContext false, and stayCandidate null; it must not use price or total_price.",
    "A fixed maximum occupancy request for one explicitly identified room or bundle must use type lodging_product_capacity, detailIntent quantity, requestedOutputs capacity, dependsOnStayContext false, and stayCandidate null. Lodging recommendation, suitability, and date-and-guest-dependent selection must keep their existing lodging capability instead.",
    "A request for the system to make a subjective choice or recommendation of a lodging product for the guest must use type human_help, detailIntent general, requestedOutputs handoff, dependsOnStayContext false, entity category other, canonicalCandidate null, and stayCandidate null so it follows the existing human handoff path. This category must not include a request for fixed maximum occupancy of an identified product, available lodging products, or lodging price or availability.",
