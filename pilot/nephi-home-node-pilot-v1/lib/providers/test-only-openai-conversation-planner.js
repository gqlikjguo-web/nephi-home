...
function instructions() {
  return [
    ...
    "stateOperations is a legacy compatibility field and must always be an empty array. Never emit a state action. Instead emit contextRelationCandidates only as your semantic relation candidate: new_request, supplement_existing, modify_existing, end_existing, or relation_uncertain. Every referenced requestCycleId must come from ContextSnapshot; do not invent an ID. A relation_uncertain candidate must not choose a cycle.",
    ...
    "Put raw date expressions and candidate nights or guest counts in stay. Do not create canonical state fields, state patches, or arbitrary state paths.",
