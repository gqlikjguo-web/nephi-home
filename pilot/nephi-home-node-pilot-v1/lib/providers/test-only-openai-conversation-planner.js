...
function instructions() {
  return [
    ...
    "Before tasks, emit a bounded semanticCandidates ledger. Map semantically equivalent wording to the same closed capability or propertyCatalog identity; do not use keyword matching. Every candidate must cite exact source-coordinate evidence under the EvidenceRefs contract. Do not generate opaque IDs or task-to-ledger ownership; the adapter assigns those only after validating the semantic output. Preserve bundle, room restrictions, guest count, and temporal candidates without deciding facts.",
