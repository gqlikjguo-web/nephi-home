...
function recordAvailabilityRead(rows, args) {
...
class SyncPostgresRpc{constructor(connection){assertConnection(connection);this.worker=new Worker(path.join(__dirname,"postgres-worker.js"),{workerData:{connection}});this.call("ready",[]);}call(name,args){const signal=new SharedArrayBuffer(8),buffer=new SharedArrayBuffer(4*1024*1024);this.worker.postMessage({name,args,signal,buffer});const status=new Int32Array(signal);const waited=Atomics.wait(status,0,0,30000);if(waited==="timed-out")throw new Error(`postgres provider timeout: ${name}`);const value=JSON.parse(Buffer.from(new Uint8Array(buffer,0,Atomics.load(status,1))).toString("utf8"));if(!value.ok){const error=new Error(value.error);error.stack=value.stack||error.stack;if(value.code)error.code=value.code;if(Number.isInteger(value.status)&&value.status>0)error.status=value.status;throw error;}if(name==="getRows")recordAvailabilityRead(value.result,args);return value.result;}close(){return this.worker.terminate();}}
...
for(const method of ["getStatus","setLimit","setAiEnabled","getHandoff","setHandoff","reserve","beginAttempt","finishAttempt","listConversations","authorizeManual","authorizeOperatorTest"]){PostgresCommercialProvider.prototype[method]=function(...args){return this.rpc.call(`commercial_${method}`,args);};}
