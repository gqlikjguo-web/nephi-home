...
- **Temporal wire shape:** `temporalCandidate` is `null` or exactly `{rawText,kind,checkInCandidate,checkOutCandidate,nightsCandidate}`. `kind` is one of `absolute_date`, `date_range`, `relative_date`, `relative_range`, `weekday`, `month_weekday`, `nights_only`, `partial`, or `unknown`; check-in/check-out are bounded strings or `null`, and nights is a positive integer or `null`. It is source-bound candidate meaning only: it never supplies an implicit year or executable date. A structurally explicit check-in/check-out/nights contradiction rejects without repair.
- **Slot wire shape:** every `slotCandidates[]` item is exactly `{slotCandidateId,slot,operation,value,evidenceRefs}`. `slotCandidateId` is turn-wide unique; `slot` is `guest_count`, `product`, `transport`, or `other_supported`; `operation` is `SET` or `CLEAR`; `value` is `string|integer|boolean|null`; and `evidenceRefs` is non-empty. `guest_count` SET requires a positive integer and every CLEAR requires `null`. Formal catalog/registry admission and semantic contradictions remain later validator responsibilities.
