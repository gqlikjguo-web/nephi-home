...
A non-null temporal candidate is a closed `{rawText,kind,checkInCandidate,checkOutCandidate,nightsCandidate}` record; it records OpenAI's source-bound temporal meaning only and cannot establish an implicit year or executable date. A slot candidate is a closed `{slotCandidateId,slot,operation,value,evidenceRefs}` record with a turn-wide stable ID. Slot candidates may express `guest_count`, `product`, `transport`, or registry-admitted `other_supported` candidates, but they cannot write state, select a Context target, alter capability, or decide reply. Closed wire validation rejects malformed fields, enums, cardinality, duplicate slot IDs, and explicit structured temporal contradictions without repair; later Evidence/Semantic/Context layers retain their respective authority.
...
The persisted conversation-state-v3 schema does not migrate. The Lifecycle Manager adapts validated C06 decisions to the existing reducer operations and reads the existing referenceable snapshot. It may persist only the same state-v3 concepts already supported. New `unitId` values are trace/request correlation identifiers and need not become DB columns. If implementation proves a required lifecycle cannot be represented without a schema change, cutover stops; the plan does not authorize migration.
