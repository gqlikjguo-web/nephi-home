...
公開照片直接使用自訂 HTTPS 圖片網域，不用 presigned URL、r2.dev 或 Render binary route。Object 設定 Cache-Control: no-store，請勿以 CDN 規則強制忽略此標頭，以免刪除後仍由 CDN 提供舊圖。這是公開住宿照片：持有圖片連結的人可查看；跨旅宿 API 管理權限仍拒絕。
...
2. **該 bucket → Settings → Custom Domains → Add**：綁定自己控制、且位於同 Cloudflare account 的網域，例如 `photos.junzanai.com`（範例，需使用實際可控制網域），等狀態 Active。不要用 r2.dev 當正式網址。
...
4. **Render → junzan-ai → Environment**：設定下列五個值，使用 Render 的安全環境變數介面。不要填入 repository 或任何前端檔案。
   ...
   - `ROOM_GALLERY_PUBLIC_BASE_URL`：正式圖片網域 origin（`https://...`，沒有 path/query）。
