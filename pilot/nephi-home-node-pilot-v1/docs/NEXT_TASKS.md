...
## 2026-08-07 opaque deployed-attribution exact-SHA acceptance

1. After the single root-cause commit is non-force pushed to `test-only/node-pilot-integration`, require exact-SHA GitHub verification and Render `ready` with `testOnly=true` before accepting any deployed result.
2. Run the exact 18-turn `target_preflight` and require every target to prove repair provenance through the same opaque correlation ID as its expected canonical capability/formal subject. `rg-023` must directly join semantic repair to canonical pool evidence; any missing, unrelated, duplicate, conflicting, unknown, or ambiguous mapping is a failure.
3. Only after exact-SHA preflight succeeds, run the complete 77-case / 90-turn real OpenAI + PostgreSQL matrix without retries and compare every prior-PASS turn. Product Baseline and real LINE remain blocked until formal acceptance; this implementation turn performs no deployment or Render/LINE operation.
