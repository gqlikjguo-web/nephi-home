...
**Reason:** Rollback artifact `8951474217` showed two prior-PASS requests in which OpenAI attached the pool-fee or extra-feature subject to a `price` task. The existing price-preservation repair correctly retained lodging price but replaced the incompatible entity with `other`, silently losing the independently expected property capability.

**Constraint:** Isolation requires verified `new_request` evidence and the Planner entity raw text must occur in that evidence. Verified raw evidence is authoritative over a conflicting Planner `canonicalCandidate`; the compiler must not synthesize or answer the conflicting candidate. It does not scan unrelated message text, use a Case ID or guest phrase, duplicate an already represented property task (including the same unresolved raw subject under another category), synthesize a catalog fact, project stay state, or execute availability before dates are ready. The property fee detail remains Unknown when formal fee data is absent; booking, high-risk, continuation, modification, ambiguity, past-date, and multi-cycle authority are unchanged.
