...
## D-049 -- Preserve source-bound stateless subjects before price normalization

**Decision:** When a verified current-event `new_request` price task contains a source-bound stateless entity category that the controlled lodging-price normalization would erase, the semantic compiler isolates that entity as an independent property task before retaining the original price task. Exact current-property catalog entities remain formal; an unsupported but controlled amenity, room-feature, activity, policy, payment, cancellation, transport, check-in, or check-out entity remains unresolved and fail closed.

**Reason:** Rollback artifact `8951474217` showed two prior-PASS requests in which OpenAI attached a source-bound stateless property subject to `price`; controlled price normalization retained the price task but erased its amenity or room-feature entity. The local repair passed deterministic RED/GREEN and complete tests.

**Constraint:** Isolation requires verified `new_request` evidence and the Planner entity raw text must occur in that evidence. Verified raw evidence is authoritative over a conflicting Planner `canonicalCandidate`; the compiler must not synthesize or answer the conflicting candidate. It does not scan unrelated message text, use a Case ID or guest phrase, duplicate an already represented property task, synthesize a catalog fact, project stay state, or execute availability before dates are ready.

## D-050 -- Roll back stateful catalog-subject isolation

**Decision:** Remove the stateful catalog-subject isolation repair and its tests, restoring the exact runtime code from commit `32ff2fe1bf7ee80f444fa595ee7e2eb43ce627a1`.
...
## D-054 -- Planner contract repair is task-level, additive, and locally fail-closed

**Decision:** A parsed OpenAI Planner response is repaired at task granularity before whole-collection rejection. Structurally and context-valid tasks are preserved byte-for-byte; an invalid sibling becomes a source-isolated `human_help` task only when its evidence is unique and does not overlap preserved evidence. Missing structured property subjects may use one bounded additive Planner round, but the second response can add only verified missing subjects and can never replace the first response. `local_contract_failure` and temporal disagreement are not provider-error retry categories.

**Reason:** Full-matrix run `31087206266` artifact `8962416807` returned HTTP 200 and parseable structured output for 17 failed cases, but whole-collection local contract validation retried the complete prompt and discarded the first response's usable tasks. This turned multi-question turns such as the price, pool, and check-in request into empty global handoffs. Targeted RED/GREEN also proved that FAQ substring matches, whole-message evidence, copied or overlapping sibling spans, and contradictory canonical candidates could otherwise authorize or suppress the wrong formal subject.

**Constraint:** Coverage discovery excludes FAQ-only matches and requires controlled source-bound ownership; a fee-shaped drift may represent only one uniquely mentioned formal subject. Evidence identity is resolved through the canonical source event before interval comparison. Stateful duplicate task IDs, ambiguous sources, unsupported temporal projection, and collections with no safe sibling scope remain fail closed. Target preflight must prove the repair boundary executed and must verify expected capability, canonical subject, and zero premature availability queries; a generic PASS trace is not attribution.

## D-055 -- Bounded coverage includes formal lodging inventory without inventing products

**Decision:** The one additive Planner coverage round may include current-property lodging inventory that is explicitly named by unique room number or catalog type. A guest-count-only request may add a bundle only when exactly one capacity-eligible bundle exists, its formal member IDs cover every enabled individual room, and the stated count exceeds every individual room capacity. A month-qualified recurring weekday on a room or bundle price task may add one availability-shaped clarification sibling only while concrete check-in and checkout candidates are absent.

**Reason:** Target preflight artifact `8973742558` eliminated whole-response contract loss but left four omissions across pool conditions, room class, room numbers, whole-property bundle scope, and date clarification. The prior coverage compiler scanned only structured property facts, so its second call could not name missing lodging subjects. The deployed safe trace also dropped the existing coverage markers at the HTTP projection layer, making successful repair attribution impossible.

**Constraint:** Inventory comes only from the current property catalog and verified current-event evidence. Ambiguous type mentions are represented as the complete formal catalog set, never an arbitrary member; explicitly removed or negated room mentions are not positive coverage subjects, including when a bounded room classifier or booking/stay verb separates the polarity word from the room label. Capacity inference cannot select a partial-room or one-of-many bundle. The first task collection remains additive and unchanged; repaired tasks must pass structural, relation, source, catalog, capability-registry, and risk checks. Each independently scoped eligible recurring-price task may receive its own companion within the collection limit, while duplicate price tasks with the same scope share one. Such companions are limited to month-plus-weekday/weekend expressions, remain subject to Temporal Resolver and query readiness, and cannot bypass past-date or ambiguity policy; an existing availability task suppresses one only for the same source relation, inventory scope, and recurring-date scope. Safe diagnostics expose only bounded booleans/counts already on the allowlist.

## D-056 -- Deployed repair attribution requires opaque direct canonical joins

**Decision:** Every Planner collection, coverage, or semantic repair that may certify target preflight receives a fresh UUID-v4 correlation ID bound privately to the exact repaired task. The safe trace exposes only an allowlisted repair kind and correlation ID; CanonicalRequest diagnostics carry the same correlation ID only on canonical evidence compiled from that task. Deployed attribution succeeds only through an exact one-to-one correlation join whose canonical capability and formal subject satisfy the target expectation.
