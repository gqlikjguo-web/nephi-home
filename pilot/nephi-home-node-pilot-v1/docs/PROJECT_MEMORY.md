...
## 2026-08-06 rollback acceptance run 31058670756 and stateful catalog isolation root

- Rollback commit `32ff2fe1bf7ee80f444fa595ee7e2eb43ce627a1` passed verify job `92481635554`; Render returned HTTP 200, `ready`, `testOnly=true`, and that exact SHA. Artifact `8951474217` has GitHub and locally verified digest `sha256:92dd21dfe489f723291f4340d9c40e5a7a144271cef013bbf7509f93d5efd4d8`.
- The complete 77-case / 90-turn matrix produced 67 PASS, 6 FAIL, and 4 non-executable cases. The rollback recovered all seven turns regressed by artifact `8950851806`, but provider variance introduced two different prior-PASS regressions: `rg-024` trace `7779c1a0-02f7-4af1-ba8e-c256271fe1c9` and `rgs-018` trace `5c7ddf98-fd67-436d-a178-62af2f3d7b77`. `rgs-017` happened to return PASS without the reverted grammar and is not considered deterministically fixed.
- In both new regressions OpenAI attached a source-bound stateless property subject to `price`; controlled price normalization retained the price task but erased its amenity or room-feature entity. The active compiler root isolates that entity before normalization only when verified current-event evidence contains the Planner raw entity text, while retaining the original price task.
- Targeted RED reproduced the one-task semantic loss. GREEN is 45/45 for semantic compiler plus a full Engine test proving price remains missing-date clarification, the independent catalog task executes, availability query count stays zero, missing fee data remains explicit, and FinalResponse does not substitute a general fact. Independent review found and blocked a raw/canonical conflict that could synthesize a second formal subject; raw evidence now wins and unresolved subjects deduplicate across categories. Property routing 16/16, canonical golden, Engine integration, semantic core 35/35, temporal, relative weekday, date range, multi-cycle, and golden matrix 22/22 pass. Complete post-review `npm test` passed in 564.2 seconds with posttest OIDC 10/10, deployed contract 18/18, and matrix contract 8/8.
