...
## 2026-08-06 deployed acceptance run 31066678489 and mandatory duplicate-source rollback

- Commit `d22e4f7d4913d2f32ab5549e1768d1a5dbd8f231` passed verify job `92505731301`; Render returned HTTP 200, `ready`, `testOnly=true`, and that exact SHA. Artifact `8954260810` has GitHub and locally verified digest `sha256:e24469e2d318dfa7c624d084812ee660da9d46ff3cb4c254d22b82a942abbb08`.
- The complete 77-case / 90-turn matrix produced 66 PASS, 1 partial-not-executable, 6 FAIL, and 4 non-executable cases. Prior-PASS regressions versus rollback artifact `8953248983` were `rg-015`, `rg-023`, and `rgs-018` turn 1; versus stable artifact `8950072765` they were those three plus `rg-038` turn 2.
- `rg-022` trace `be8980d3-f5f6-456b-baf6-2486bae35bdf` returned PASS but did not execute `duplicate_source_canonical_authority`; the Planner directly emitted policy with no canonical candidate. The repair instead executed on already-PASS `rg-016`, `rg-017`, `rg-019`, `rg-020`, and `rgs-003`. The target recovery is unproven sampling drift and the artifact is rejected.
- The active worktree restores runtime and tests to `7f9edf2a4ef6c055cf4bda0a67b03b625abfc0de` while retaining all experiment and deployment evidence. Real LINE acceptance remains blocked.
- The exact rollback passed complete local `npm test` in 719.4 seconds with 3,883 output lines and exit 0; posttest OIDC was 10/10, deployed contract 18/18, and matrix contract 8/8.

## 2026-08-06 rollback acceptance run 31068352292 and resolved lodging scope root

- Rollback commit `f54c644f8f85303bd0490aeab5b6cc2234d56f8d` passed verify job `92510857312`; Render returned HTTP 200, `ready`, `testOnly=true`, and that exact SHA. Artifact `8954889733` has GitHub and locally verified digest `sha256:a62d0994136abae2c4bc7dca826df8e6944d182b51be5442504db670e7f81abd`.
- The complete matrix produced 66 PASS, 7 FAIL, and 4 non-executable cases. It recovered rejected-artifact regressions at `rg-015`, `rg-038` turn 2, and `rgs-018` turn 1, but provider variance introduced prior-PASS regressions at `rg-006`, `rg-023`, `rg-039` turn 1, `rgs-018` turn 2, and `rgs-019` turn 2 versus rollback artifact `8953248983`.
- The earliest failure is `rg-006` trace `86fb7f14-eef6-4e64-b488-a94717b31a73`. Planner validation had a resolved room entity and availability capability, but `room_or_bundle_restriction` detail drift caused generic policy normalization to erase the room, execute the property catalog, and override the correct past-date clarification with a handoff.
- Targeted RED reproduced this without a guest phrase or case identifier. GREEN preserves only uniquely resolved room or bundle inventory, emits `resolved_inventory_detail_scope_preservation`, retains `past_date`, and keeps the pre-existing unresolved-room policy negative control. Planner semantic is 38/38; failure safety, temporal authority/range, common-root, dialogue state, per-request, multi-cycle, semantic core 35/35, and golden matrix 22/22 pass.
- Focused read-only review returned Ready with no Critical or Important finding. Complete local `npm test` passed in 346.4 seconds with 3,883 output lines and exit 0; posttest OIDC was 10/10, deployed contract 18/18, and matrix contract 8/8.

## 2026-08-06 full-matrix run 31087206266 and task-level Planner recovery candidate

- Commit baseline `83dbeb8340501171fa66654fb817bb886b5da145` produced private artifact `8962416807`: 57 PASS, 16 FAIL, and 4 non-executable cases; 63 PASS, 18 FAIL, and 9 non-executable turns. Seventeen failed turns had HTTP 200, a response body, and parsed structured output, but two complete attempts ended in `local_contract_failure` and discarded all tasks. `rg-023` separately lost the formal pool-fee subject during price normalization.
- The reviewed candidate removes local/temporal contract disagreement from provider retries, preserves valid task/relation siblings, source-isolates invalid siblings, adds one bounded additive missing-subject round, and normalizes source-bound pool-fee drift without weakening Temporal Resolver, duplicate-ID, QueryPlan, or resolver authority.
- Negative controls cover FAQ-only and external-place substrings, whole-message omitted subjects, contradictory canonical candidates, multiple formal subjects, copied/contained/partially overlapping evidence, cross `eventId`/`messageRef` identity, overflow, failed repair, stateful duplicates, and premature availability attribution. Focused tests pass; final read-only review returned Ready with no Critical or Important finding.
- The candidate is not yet a deployed baseline. Next required evidence is a fresh complete local suite and safety gates, one root-cause commit, non-force push, exact-SHA GitHub/Render verification, the attributed 18-turn real preflight, then the complete 77-case / 90-turn OpenAI + PostgreSQL matrix. Real LINE remains blocked.

## 2026-08-06 target preflight run 31115977348 and lodging coverage continuation

- Commit `23f107c38e284aaad975de3d74c98a13c0449fb4` passed verify job `92665494885`; Render returned HTTP 200, `ready`, `testOnly=true`, and that exact SHA. Target artifact `8973742558` has verified digest `sha256:04fc6d882158b78345c2d729ba015d6b270728e9ef57d56a4df254ace06929ad`.
- The exact 18-turn preflight produced 14 PASS and 4 FAIL turns across 16 executable cases. All 17 prior `planner_semantic_path_required` / `local_contract_failure` turns were eliminated, and `rg-023` passed through the required property-catalog pool grounding.
- Remaining failures share a Planner coverage boundary: `rg-037` omitted the pool usage condition beside price, `rg-038` omitted an availability/date-clarification sibling for a month-qualified recurring-day room-price request, `rgs-019` omitted the only formal whole-property bundle able to contain the stated guest count, and `rgs-020` omitted explicitly stated room numbers beside the bundle.
- The active continuation extends the bounded additive repair only through the current property catalog: explicit inventory type/number mentions, or one uniquely eligible bundle whose formal members cover every individual room and whose capacity is required by the current guest count. Month-qualified recurring weekdays with no concrete date receive one additive clarification capability. Safe trace projection now retains coverage/task-collection attribution without exposing message text or provider bodies.
- Focused RED/GREEN and the complete local `npm test` pass on the active continuation: exit 0 in 390.7 seconds with 3,883 output lines; posttest OIDC is 10/10, deployed contract 18/18, and matrix contract 8/8. This is not a deployed baseline; Reviewer Ready, commit, non-force push, exact-SHA preflight, and then the complete matrix remain required. Real LINE remains blocked.
- Final review closure added bounded negation/removal handling, per-scope recurring-date companions, accumulated-relation deduplication, and regressions for natural Chinese/English exclusions, unrelated rooms, multiple independent room scopes, and duplicate same-scope price tasks. Independent review returned **Reviewer Ready** with no Important or Must Fix finding. The final bytes passed complete `npm test` in 453.3 seconds with 3,883 lines and exit 0, plus protected acceptance 17/17, Codex integrity 36/36, Constitution suites, runtime uniqueness 52/52 with no mutation, canonical golden, `git diff --check`, and the focused planner/safety/trace suites. Commit, non-force push, and deployed evidence are still pending; this remains outside Product Baseline.

## 2026-08-07 opaque deployed-attribution closure

- The active candidate replaces detached repair markers with fresh UUID-v4 correlations privately bound to the exact collection fallback, coverage addition, or semantic repair task and directly projected onto that task's canonical diagnostic evidence.
- Deployed target validation now requires an exact one-to-one join and expected capability/formal-subject match. Missing provenance, unrelated repair, unknown or duplicate IDs, one-to-many evidence, conflicting kinds, and one canonical task claiming different correlations all fail closed. `rg-023` uses the same semantic-repair-to-pool join.
- Safe provenance contains only allowlisted repair kind/correlation pairs. Raw semantic validation task IDs are not projected, and `validation` is the only safe semantic provenance source. Formal CanonicalRequest objects and product task results remain unchanged.
- Focused Planner, Semantic, Engine, server, persisted-trace, and deployed-contract tests pass on the reviewed candidate. Independent review is **Reviewer Ready** with no Critical, Important, or Minor finding. No fixture, acceptance matrix, Planner behavior, Product Baseline, Render, or LINE state was changed.
