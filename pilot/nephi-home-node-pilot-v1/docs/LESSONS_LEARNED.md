...
## 2026-08-06 -- Property-fact coverage does not cover lodging inventory

A bounded missing-subject repair can eliminate local contract failures and still omit rooms or bundles because property facts and sellable inventory have different catalog and resolver authorities. A price task can also swallow a facility usage condition, while a broad month-plus-weekday price request needs a separate clarification capability even though no availability query is ready.

Compile coverage from the current formal catalog rather than guest-case phrases. Preserve ambiguous room types as the complete matching set, accept explicit room numbers only when at least one occurrence is not negated or removed, including bounded natural forms such as a polarity word followed by a room classifier or booking/stay verb, and infer a bundle from guest count only when it is the sole whole-property formal product that can satisfy the count. Date clarification must be considered per eligible price task, while deduplication compares source relation, inventory scope, and recurring-date scope; the mere presence of another availability task is not equivalent coverage. Keep the addition source-bound and registry-validated. Finally, verify every repair marker survives each safe diagnostic projection layer; an internal marker that disappears at the HTTP boundary cannot prove deployed attribution.

## 2026-08-12 -- Real OpenAI tests require prior case review

Before any real OpenAI test is executed, the user must first review the complete set of test questions and their expected product behavior. Local deterministic tests, release verification, or a successful deployment do not authorize a real OpenAI run, rerun, targeted preflight, or full matrix without that explicit review.
