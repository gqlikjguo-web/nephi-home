...
async function run(dispositions, fixtureOptions = {}) {
  ...
  const facts = dispositions.map((_, i) => ({ canonicalId: "fixture_" + randomBytes(6).toString("hex"), category: answer.policy.subjectKinds[0],
    publicName: `Fixture fact ${i}`, status: fixtureOptions.unknownFacts ? "unknown" : "allowed", publicText: `Official fixture fact ${i}` }));
  ...
  const result = await executeNewCoreTurn({
    ...
    understandingProvider: (input, options) => callOpenAIUnderstandingV1(input, { ...options, nowMs: () => Date.parse(now), fetchImpl: async () => {
      ...
      const units = dispositions.map((disposition, i) => {
        ...
        if (disposition === "NO_REPLY") return { unitId: `unit-${i}`, evidenceRefs: [ref], purpose: fixtureOptions.purpose || "conversational_statement", capability: null, subject: {kind:null,catalogIdentity:null}, stayDependent:false, temporalCandidate:null, contextLinkCandidateId:`link-${i}`, safetyCandidate:null, slotCandidates:fixtureOptions.withSlot ? [{slotCandidateId:`slot-${i}`,slot:"guest_count",operation:"SET",value:7,evidenceRefs:[ref]}] : [], confidenceBand:"high" };
