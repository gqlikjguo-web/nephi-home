...
const specs=[];
for(const fault of ['priceOverrides','dateClassifications']) for(const units of [['A','H'],['C'],['H'],['N'],['N','A']]) specs.push({id:units.join('-')+'-'+fault,units,fault});
for(const units of [['H'],['C'],['N'],['C','H'],['A','H']]) specs.push({id:units.join('-')+'-rules',units,fault:'customReplies'});
(async()=>{const rows=[];for(const spec of specs){const r=await run(spec);let problem;
try{assert.equal(r.error,undefined);assert.equal(r.calls,1);assert.ok(r.outcomes);assert.equal(r.validation?.ok,true);
if(spec.units.includes('N')&&spec.units.length===1)assert.equal(r.decision.action,'no_reply');
if(spec.units.includes('H'))assert.ok(r.outcomes.some(x=>x.type==='human_help'&&x.reason==='human_help'));
if(spec.units.includes('C'))assert.ok(r.outcomes.some(x=>x.outcome==='not_ready'));
if(spec.units.includes('A')&&spec.fault!=='customReplies')assert.ok(r.outcomes.some(x=>x.outcome==='answered'));
if(spec.units.includes('A')&&spec.fault==='customReplies')assert.ok(r.outcomes.some(x=>x.outcome==='technical_error'));
assert.equal(r.reads.includes('priceOverrides'),false);assert.equal(r.reads.includes('dateClassifications'),false);
