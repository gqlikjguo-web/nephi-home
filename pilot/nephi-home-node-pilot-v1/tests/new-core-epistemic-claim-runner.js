...
function validate(tasks){const plan=buildResponsePlan({propertyId:'fixture',taskResults:tasks,inputTaskIds:tasks.map(t=>t.taskId)});return {plan,validation:validateClaims(composeControlledReply(plan),plan,tasks.map(t=>t.taskId))};}
...
test('U6 epistemic unknown without provenance is rejected',async()=>{
  ...
  assert.equal(validate([original]).validation.ok,true);
  ...
  const result=validate([withoutProvenance]);
...
test('U5 mixed fact and unknown validate independently',async()=>{const {records}=await run(['ANSWER'],{unknownFacts:true});const unknown=records.planInputs[0].taskResults[0];const good=validate([factual(),unknown]);assert.equal(good.validation.ok,true,JSON.stringify(good.validation));assert.equal(good.plan.sections[1].claimType,'EPISTEMIC_UNKNOWN');assert.ok(validate([factual(false),unknown]).validation.errors.includes('missing_fact_source'))});
