...
test('U1 official Unknown keeps validated epistemic claim without unauthorized public wording',async()=>{const {result,records}=await run(['ANSWER'],{unknownFacts:true});const {plan,validation}=validate(records.planInputs[0].taskResults,records.planInputs[0].propertyId);assert.equal(plan.sections[0].claimType,'EPISTEMIC_UNKNOWN');assert.equal(validation.ok,true,JSON.stringify(validation));assert.equal(result.finalDecision.internalAction,'reply');assert.equal(result.finalDecision.action,'no_reply');assert.equal(result.finalResponse.replyText,'');assert.equal(result.finalResponse.shouldReply,false)});
