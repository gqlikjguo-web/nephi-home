...
async function run() {
  ...
  await operator.load();
  assert.equal(writes.length, 0); assert.equal(field("aiEnabled").checked, true); assert.match(field("switchLabel").textContent,/開啟/); assert.match(textTree(field("usageWindow")),/2.*5.*80/);
  ...
  assert.match(textTree(field("usage")), /80.*100.*20/); assert.match(field("period").textContent, /2026-09/); assert.match(field("quotaStatus").textContent, /80%/);
  ...
  assert.equal(field("handoff").disabled,true,"detail action waits for the selected guest's official state");
  ...
  limit = null; await operator.load(); assert.match(field("quotaStatus").textContent, /額度未設定/); assert.doesNotMatch(textTree(field("usage")), /無限/);
  ...
  assert.match(textTree(field("usage")), /100/); assert.equal(field("handoff").disabled, true);
