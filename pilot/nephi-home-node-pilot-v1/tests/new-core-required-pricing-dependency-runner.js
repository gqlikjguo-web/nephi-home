...
for(const fault of [null,'priceOverrides','datePriceClassifications']) {
 let priceCalls=0,dateCalls=0;
 const r=executeQueryPlan({property:pricingProperty,catalog,queryPlan:pricingPlan,
 ...
 priceOverrides:()=>{priceCalls++;if(fault==='priceOverrides')throw Error('required price failure');return [];},
 datePriceClassifications:()=>{dateCalls++;if(fault==='datePriceClassifications')throw Error('required date failure');return [];}});
 assert.equal(priceCalls,1);assert.equal(dateCalls,fault==='priceOverrides'?0:1);
 assert.equal(r.outcome,fault?'technical_error':'answered');
 if(fault){assert.equal(r.reason,'resolver_exception');assert.deepEqual(r.facts,{});}
 const sibling=executeQueryPlan({property,catalog:propertyCatalog,queryPlan:propertyFactPlan,priceOverrides:()=>{throw Error('not required');},datePriceClassifications:()=>{throw Error('not required');}});
 assert.equal(sibling.outcome,'answered');
}
console.log('RUNTIME_COMPONENT_TEST required pricing dependency/sibling cases PASS');
