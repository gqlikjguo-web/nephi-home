...
(async () => {
  ...
  assert.match(bundleSubmitSource, /method:\s*id\s*\?\s*"PUT"\s*:\s*"POST"/, "bundle UI must use the existing endpoint for create and update");
  for (const field of ["name", "capacity", "memberRoomIds", "entertainmentAmenities", "enabled"]) assert.match(bundleSubmitSource, new RegExp(`${field}:`), `bundle UI payload must include ${field}`);
  assert.match(adminScript, /async function deleteBundle/, "bundle UI must expose the existing property-scoped delete path");
  assert.match(adminHtml, /id="bundleForm" hidden/, "bundle editor must stay hidden until an existing bundle is selected");
  assert.match(adminHtml, /id="bundleId" type="hidden"/, "bundleId must remain hidden and server-generated");
  assert.doesNotMatch(adminHtml, /id="bundleName" disabled/, "bundle name must be editable");
  assert.doesNotMatch(adminHtml, /<fieldset disabled><legend>包含房型/, "bundle members must be editable");
  assert.doesNotMatch(adminHtml, /id="bundleEnabled" type="checkbox" disabled/, "bundle enabled state must be editable");
  assert.match(adminHtml, /id="bundleCreate"/, "bundle UI must expose an explicit create action");
  ...
  assert.match(adminScript, /\\u5e74.*\\u6708.*\\u6574\\u6708\\u5feb\\u901f\\u8a2d\\u5b9a/, "month mode must identify the selected year and month");
  assert.match(adminScript, /value === "closed" && !confirm/, "monthly close must require confirmation before its API call");
  assert.match(adminScript, /room\.name.*year.*month/, "monthly close confirmation must identify inventory and month");
  assert.match(adminScript, /--inventory-count/, "monthly and daily grids must share an explicit inventory column count");
  const monthlyControlSource = adminScript.slice(adminScript.indexOf("function renderMonthlyInventoryControls"), adminScript.indexOf("function createDayCard"));
  assert.equal((monthlyControlSource.match(/\/api\/availability\/month/g) || []).length, 1, "each monthly action must retain one generic month request");
