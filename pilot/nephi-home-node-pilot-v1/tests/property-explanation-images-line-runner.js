...
(async()=>{
 for(const mode of ['bound','foreign','unbound','storage-failure']){
  ...
  const app=require('../server').createApp({providers,enableProductionLineEngine:true,testOnlyEnvironment:false,runtimeEnv:{OPENAI_API_KEY:crypto.randomBytes(24).toString('hex')},lineBindingEnv:binding.lineBindingEnv,conversationDebounceMs:1,
   getPropertyImageStore:async()=>({attachments:async(propertyId,sources)=>{assert.equal(propertyId,'image_a');assert.deepEqual(sources,['parking']);if(mode==='storage-failure')throw Error('synthetic media read failure');return [image];}}),
 ...
 console.log('PASS same LINE reply text+image, foreign/unbound/failure text-only safety; FAKE_INTEGRATION; OPENAI_CALLS=0; REAL_LINE=NOT_RUN');
