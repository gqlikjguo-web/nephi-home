...
for(const kinds of [['A'],['C'],['P'],['H'],['T'],['A','C'],['A','H'],['A','C','H'],['A','P','H'],['SUPPRESSED'],['ABSENT']])test(kinds.join('+'),()=>{const r=finish(kinds);assert.equal(r.rebuildCount,0);assert.ok(Array.isArray(r.responsePlan.renderObligations),'typed obligations required');assert.equal(r.finalResponse.shouldReply,!['ABSENT','SUPPRESSED','T'].includes(kinds[0]));if(r.finalResponse.shouldReply)assert.equal(r.claimValidation.ok,true);});
