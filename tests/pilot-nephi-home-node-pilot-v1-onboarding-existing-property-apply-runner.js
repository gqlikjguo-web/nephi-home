...
(async () => {
  ...
  try {
    ...
    result = await approve("apply_wrong_target", { ...validPayload(), propertyId: "other_property", confirmPropertyId: "other_property" }); check("固定 target 與多 property 隔離", result.response.status === 409 && result.body.error.code === "EXISTING_PROPERTY_TARGET_NOT_ALLOWED");
    ...
    result = await request(`${base}/api/admin/onboarding/properties`, { headers: { cookie } }); check("後端只提供固定 target 與穩定 inventory ID", result.response.status === 200 && result.body.data.items.length === 1 && result.body.data.items[0].propertyId === "nephi_home" && result.body.data.items[0].rooms.some((room) => room.id === "room301") && result.body.data.items[0].bundles.some((item) => item.id === "bundle_four_room_whole_house"));
